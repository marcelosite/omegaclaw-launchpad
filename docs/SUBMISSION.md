...
Launchpad Studio extends that path for a nontechnical user: its human-first Wizard explains the referee story in plain language, blocks each Next step until the required evidence exists, labels where every command runs, offers the synthetic `community-care` lesson, copies a readable workspace, and provides a bounded local MCP handoff with only `omega.reason` and `omega.get_receipt`. The MCP also includes one closed Community Hospital first review teaching packet so an agent can submit a structured disagreement without turning the bridge into an unrestricted multi-agent service.
...
- Real pinned-runtime community-care lesson proof using Test/WebSocket/MeTTa/NAL.
