"""End-to-end proof for Launchpad's First Reflection contract.

This file is copied into OmegaClaw-Core/Autotests/mock_websocket by the proof
runner.  A passing test means the WebSocket channel, Test provider, agent loop,
and real MeTTa/NAL skill all ran inside the pinned OmegaClaw runtime.
"""

import json
import os
import subprocess
import time
from pathlib import Path

from helpers import Checker, make_prompt, wait_for_skill_call
from ws_helpers import ws_send_prompt


CONTAINER = os.environ.get("OMEGACLAW_CONTAINER", "omegaclaw")
MISSION_ROOT = Path(os.environ["LAUNCHPAD_MISSION_ROOT"])


def _docker_logs():
    result = subprocess.run(
        ["docker", "logs", CONTAINER], capture_output=True, text=True, check=False
    )
    return (result.stdout or "") + (result.stderr or "")


def _wait_for_reply(ws, needle, timeout=60):
    deadline = time.time() + timeout
    while time.time() < deadline:
        _client_seq, text = ws.pop_agent_reply(timeout=max(1, int(deadline - time.time())))
        if text is None:
            break
        if needle in text:
            return text
    return None


def _wait_for_nal_result(marker, timeout=60):
    deadline = time.time() + timeout
    while time.time() < deadline:
        lines = [
            line
            for line in _docker_logs().splitlines()
            if "CHARS_SENT:" in line
            and "LAST_SKILL_USE_RESULTS:" in line
            and marker in line
            and "stv" in line
        ]
        if lines:
            return lines[-1]
        time.sleep(2)
    return None


def test_launchpad_first_reflection_uses_real_nal(llm, ws):
    with Checker("Launchpad First Reflection via real OmegaClaw") as check:
        context_path = MISSION_ROOT / "03-reflection" / "reflection-context.json"
        context = json.loads(context_path.read_text(encoding="utf-8"))
        finding = context["validated_findings"][0]
        marker = "launchpad_claim_%s" % check.run_id
        reply_token = "OMEGACLAW-REFLECTION-%s" % check.run_id

        check.step("wait for OmegaClaw WebSocket connection")
        if not ws.wait_for_connection(timeout=60):
            check.fail("connection", "OmegaClaw did not connect to the local WebSocket harness")
        check.ok("connection")
        ws.clear()

        check.step("wait for the OmegaClaw Test provider to connect")
        if not llm.ping(timeout=60):
            check.fail("Test provider", "OmegaClaw did not connect to the local LLM controller")
        check.ok("Test provider")

        prompt = make_prompt(
            check.run_id,
            "Reflect on verified evidence. Expected distinct sources: %s; observed: %s. "
            "Use NAL revision, distinguish observation from inference, and do not apply changes."
            % (finding["expected"], finding["observed"]),
        )
        metta_expression = (
            "(|- ((--> %s supported) (stv 1.0 0.45)) "
            "((--> %s supported) (stv 0.0 0.9)))" % (marker, marker)
        )
        check.step("register the controlled provider response")
        answer_registered = llm.set_answer(
            prompt,
            '(metta "%s") (send "%s: conflicting evidence; require human review.")'
            % (metta_expression, reply_token),
        )
        if not answer_registered:
            check.fail("mock answer", "the controlled skill sequence was not registered")
        check.ok("mock answer", "controlled provider response registered")

        check.step("deliver verified facts through the real WebSocket channel")
        ws_send_prompt(ws, prompt)
        check.ok("delivered")

        check.step("verify OmegaClaw invoked the real metta skill")
        metta_arg = wait_for_skill_call(check.run_id, "metta", timeout=60, arg_substr=marker)
        if metta_arg is None:
            check.fail("metta invoked", "no matching metta skill call was recorded")
        check.ok("metta invoked", metta_arg[:100])

        check.step("verify the NAL result entered the next loop context")
        nal_log = _wait_for_nal_result(marker)
        if nal_log is None:
            check.fail("NAL result", "no stv result reached LAST_SKILL_USE_RESULTS")
        check.ok("NAL result", "stv result captured in the real loop")

        check.step("verify the response returned through OmegaClaw")
        response = _wait_for_reply(ws, reply_token)
        if response is None:
            check.fail("response", "no OmegaClaw response returned through WebSocket")
        check.ok("response", response)

        proof = {
            "schema_version": 1,
            "status": "verified",
            "runtime": "OmegaClaw-Core v0.1.19-dirty",
            "upstream_base_commit": "642c53676cf795cb7a0030823b36018c029b1416",
            "controlled_patches": [
                "limit FAISS build parallelism to 2 (build only)",
                "map Linux POLLRDHUP to POLLHUP on macOS (test harness only)",
            ],
            "provider": "Test",
            "channel": "websocket",
            "metta_skill_observed": True,
            "nal_stv_observed_in_loop": True,
            "human_approval_still_required": True,
            "response": response,
        }
        proof_path = MISSION_ROOT / "03-reflection" / "omega-proof.json"
        proof_path.write_text(json.dumps(proof, indent=2) + "\n", encoding="utf-8")
        (MISSION_ROOT / "03-reflection" / "omega-response.txt").write_text(
            response + "\n", encoding="utf-8"
        )
        check.done()
