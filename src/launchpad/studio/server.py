...
function canNext(){return [document.getElementById('ack-boundary')?.checked,true,status.preflight?.state==='ready',status.proof?.state==='verified',document.getElementById('ack-synthetic')?.checked,workspaceCreated,document.getElementById('ack-connect')?.checked,document.getElementById('ack-policy')?.checked][active]!==false;}
...
async function refresh(){const response=await fetch('/api/status');status=await response.json();cardList('preflight-card',[['Workshop checks',status.preflight],['Pinned proof',status.proof],['First receipt',status.receipt]]);cardList('proof-card',[['Workshop preflight',status.preflight],['Real OmegaClaw proof',status.proof],['Synthetic lesson proof',status.factory_fault]]);cardList('template-card',[['Template files',status.template]]);const finish=document.getElementById('finish-card');finish.replaceChildren();finish.append(stateRow('Current handoff',status.handoff));}
